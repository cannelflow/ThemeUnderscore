...
if ( ! function_exists( 'myproperscore_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function myproperscore_posted_on() {
...
if ( ! function_exists( 'myproperscore_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function myproperscore_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'myproperscore' ) );
		if ( $categories_list && myproperscore_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'myproperscore' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'myproperscore' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'myproperscore' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'myproperscore' ),
 ...
 * @return bool
 */
function myproperscore_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'myproperscore_categories' ) ) ) {
		...
		set_transient( 'myproperscore_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so myproperscore_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so myproperscore_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in myproperscore_categorized_blog.
 */
function myproperscore_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'myproperscore_categories' );
}
